...
from iriscc.datautils import crop_domain_from_ds, standardize_longitudes
from iriscc.plotutils import plot_histogram
from iriscc.settings import COLORS, CONFIG, EOBS_RAW_DIR, GCM_RAW_DIR, GRAPHS_DIR, PREDICTION_DIR, RCM_RAW_DIR, SAFRAN_REFORMAT_DIR
...
if args.exp == "exp3":
    safran = xr.open_mfdataset(np.sort(glob.glob(str(SAFRAN_REFORMAT_DIR / "tas*reformat.nc"))), combine="by_coords").sel(time=slice("2000", "2015"))
...
if simu == "gcm":
    data = xr.open_dataset(glob.glob(str(GCM_RAW_DIR / "CNRM-CM6-1/tas*historical_r1i1p1f2*.nc"))[0]).sel(time=slice("2000", "2015"))
    data = standardize_longitudes(data)
    data = crop_domain_from_ds(data, CONFIG[args.exp]["domain"])
    name = "GCM 1°"

elif simu == "rcm":
    data = xr.open_dataset(glob.glob(str(RCM_RAW_DIR / "ALADIN_reformat/tas*historical_r1i1p1f2*.nc"))[0]).sel(time=slice("2000", "2015"))
    simu_name = "RCM 12km"
...
plot_histogram([tas_list[0], tas_list[2]], axes1[0], labels=[labels[0], labels[2]], colors=[colors[0], colors[2]], xlabel=None)
plot_histogram([tas_list[1], tas_list[2]], axes1[1], labels=[labels[1], labels[2]], colors=[colors[1], colors[2]], xlabel=None)
...
plot_histogram([tas_list[0], tas_list[3]], axes2[0], labels=[labels[0], labels[3]], colors=[colors[0], colors[3]], xlabel=None)
plot_histogram([tas_list[1], tas_list[3]], axes2[1], labels=[labels[1], labels[3]], colors=[colors[1], colors[3]], xlabel=None)
