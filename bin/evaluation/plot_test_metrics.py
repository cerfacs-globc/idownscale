...
from iriscc.settings import CONFIG, GRAPHS_DIR, METRICS_DIR
...
dates = pd.date_range(start=args.startdate, end=args.enddate, freq="M" if args.scale == "monthly" else "D")
...
colors = [
    ...
    "#a1d99b",
    "#41ab5d",
    "#006d2c",  # Vert clair -> foncé
    "#ffeda0",
    "#feb24c",
    "#d45f00",
    "#fc9272",
    "#de2d26",
    "#a50f15",  # Rouge clair -> foncé
]
fig, ax = plot_map_contour(
    rmse_spatial,
    domain=domain,
...
ax.text(
    0.03,
    0.07,
    f"Mean spatial RMSE: {np.nanmean(rmse_spatial):.2f}",
...
ax.text(
    0.03,
    0.07,
    f"Mean spatial Bias: {np.nanmean(bias_spatial):.2f}",
    transform=ax.transAxes,
    fontsize=10,
    verticalalignment="top",
    zorder=10,
