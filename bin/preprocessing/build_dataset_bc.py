...
from iriscc.settings import DATASET_BC_DIR, DATES_BC_TEST_FUTURE, DATES_BC_TEST_HIST, DATES_BC_TRAIN_HIST
...
if __name__ == "__main__":
    ...
    def process_date_bc(date, get_data, args, domain):
        ...
        if args.simu == "gcm":
            ...
            ds_simu = interpolation_target_grid(ds_simu, ds_target=crop_domain_from_ds(ds_gcm, domain), method="conservative_normed", bounds_method="2")
    ...
    results = Parallel(n_jobs=args.n_jobs)(
        delayed(process_date_bc)(date, get_data, args, domain) for date in tqdm(DATES_BC_TRAIN_HIST, desc="Processing Train Hist")
    )
    era5_train_hist = np.stack([r[0] for r in results], axis=0)
    simu_train_hist = np.stack([r[1] for r in results], axis=0)
    train_hist = {"era5": era5_train_hist, args.simu: simu_train_hist, "dates": DATES_BC_TRAIN_HIST}
    ...
    results_test = Parallel(n_jobs=args.n_jobs)(
        delayed(process_date_bc)(date, get_data, args, domain) for date in tqdm(DATES_BC_TEST_HIST, desc="Processing Test Hist")
    )
    era5_test_hist = np.stack([r[0] for r in results_test], axis=0)
    simu_test_hist = np.stack([r[1] for r in results_test], axis=0)
    test_hist = {"era5": era5_test_hist, args.simu: simu_test_hist, "dates": DATES_BC_TEST_HIST}
    ...
    def process_date_future(date, get_data, args):
        if args.simu == "gcm":
            ...
            ds_simu = interpolation_target_grid(ds_simu, ds_target=ds_gcm, method="conservative_normed", bounds_method="2")
        val = ds_simu[args.var].values
        ds_simu.close()
        return val

    simu_test_future = Parallel(n_jobs=args.n_jobs)(
        delayed(process_date_future)(date, get_data, args) for date in tqdm(DATES_BC_TEST_FUTURE, desc="Processing Test Future")
    )
    simu_test_future = np.stack(simu_test_future, axis=0)
    test_future = {args.simu: simu_test_future, "dates": DATES_BC_TEST_FUTURE}
