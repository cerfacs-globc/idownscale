...
def get_model(model: str, in_channels: int, out_channels: int, img_size: tuple, dropout: float = 0):

    match model:
        case "unet":
            return UNet(spatial_dims=2, in_channels=in_channels, out_channels=out_channels, dropout=dropout, channels=(32, 64, 128, 256, 512), strides=(2, 2, 2, 2)).float()
        case "miniunet":
        ...
        case "miniswinunetr":
            return MiniSwinUNETR(img_size=img_size, in_channels=in_channels, out_channels=out_channels, spatial_dims=2, drop_rate=dropout).float()
...
class IRISCCLightningModule(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if self.loss_name == "masked_gamma_mae":
            self.loss = MaskedGammaMAELoss(ignore_value=self.fill_value, sample_dir=hparams["sample_dir"])
        else:
        ...
        self.model = get_model(model=hparams["model"], in_channels=self.in_channels, out_channels=1, img_size=self.img_size, dropout=self.dropout)
