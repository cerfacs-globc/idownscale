...
from iriscc.settings import COLORS, CONFIG, DATES_BC_TEST_HIST, DATES_TEST, GRAPHS_DIR, METRICS_DIR
...
for col in list_data_mean[0].columns:
    new_df_mean = pd.DataFrame({name: df[col] for name, df in zip(test_names, list_data_mean)}).T
...
metrics_dict = {}
for col in metrics:
    new_df = pd.DataFrame({name: dict[col] for name, dict in zip(df_names, list_data)})
