...
from iriscc.datautils import crop_domain_from_ds, interpolation_target_grid, remove_countries, standardize_dims_and_coords, standardize_longitudes
from iriscc.plotutils import plot_map_contour
from iriscc.settings import CONFIG, ERA5_DIR, GCM_RAW_DIR, GRAPHS_DIR, PREDICTION_DIR, SAFRAN_REFORMAT_DIR

date = pd.Timestamp("2014-12-31 00:00:00")
safran = xr.open_mfdataset(np.sort(glob.glob(str(SAFRAN_REFORMAT_DIR / "tas*reformat.nc"))), combine="by_coords")
...
gcm = xr.open_dataset(glob.glob(str(GCM_RAW_DIR / "CNRM-CM6-1/tas*ssp585_r1i1p1f2*.nc"))[0])
...
ds_era5_to_gcm = interpolation_target_grid(ds_era5, ds_target=gcm, method="conservative_normed")
tas_era5 = ds_era5_to_gcm.tas.values

date = pd.Timestamp("2070-01-01 00:00:00")
gcm_unet = xr.open_dataset(glob.glob(str(PREDICTION_DIR / "tas*ssp585_r1i1p1f2*exp3_unet_all_gcm_bc.nc"))[0])
gcm_unet = gcm_unet.sel(time=gcm_unet.time.dt.date == date.date()).isel(time=0)
tas_unet = gcm_unet.tas.values
gcm_unet.close()

gcm_swinunet = xr.open_dataset(glob.glob(str(PREDICTION_DIR / "tas*ssp585_r1i1p1f2*exp3_swinunet_all_gcm_bc.nc"))[0])
...
plot_map_contour(
    ...
    title="tas SAFRAN 8km 2014-12-31",
    cmap="OrRd",
    var_desc="K",
    levels=levels,
...
plot_map_contour(
    ...
    data_projection=CONFIG["exp3"]["data_projection"],
    fig_projection=CONFIG["exp3"]["fig_projection"],
    title="tas SwinUNETR 8km 2070-01-01",
    cmap="OrRd",
    var_desc="K",
    levels=levels,
