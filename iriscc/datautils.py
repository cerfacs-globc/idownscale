...
def interpolation_target_grid(ds:xr.Dataset, 
   ...
   if method == 'bilinear':
      ...
      # This overwrites potentially low-precision or incorrectly shaped bounds from source files.
      if 'lon' in ds.coords:
...
class Data(object):
   ...
   def get_era5_dataset(self, var:str, date, lapse_rate_correction:bool=False, orog_target_file:str=None):
      ...
      if self.domain and self.domain[0] == -12.5:
          ds = ds.sortby('lat', ascending=True)
      elif self.domain and self.domain[0] == -6.0:
   ...
   def get_gcm_dataset(self, var:str, date, ssp:str=None, lapse_rate_correction:bool=False, orog_target_file:str=None):
      import xesmf as xe
      ...
      if self.domain and self.domain[0] == -6.0:
          ds = ds.sortby('lat', ascending=False)
      elif self.domain and self.domain[0] == -12.5:
          ds = ds.sortby('lat', ascending=True)
