...
from iriscc.settings import CONFIG, DATASET_BC_DIR, DATES_BC_TEST_FUTURE, DATES_BC_TEST_HIST, DATES_BC_TRAIN_HIST, GCM_RAW_DIR, GRAPHS_DIR, RCM_RAW_DIR


def plot_tprofiles_short_range(y: Optional[np.ndarray], x: np.ndarray, z: np.ndarray, title: str, savedir: str, simu: Optional[str] = None) -> None:
    """
...
def plot_seasonal_hist(y: list | None, x: list, z: list, dates: list, title: str, savedir: str, simu: Optional[str] = None) -> None:
    """
...
def monthly_mean(y: Optional[np.ndarray], x: np.ndarray, z: np.ndarray, dates: List[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
...
if __name__ == "__main__":
    ...
    get_data_bc = Data([-12.5, 27.5, 31.0, 71.0])  # Europeen domain
    ...
    var_marginal_bias_data = marginal.calculate_marginal_bias(
        metrics=[metrics.cold_days, metrics.warm_days], percentage_or_absolute="absolute", obs=test_hist["era5"], raw=test_hist[simu], CDFt=test_hist_bc
    )
    ...
    plt.plot(df_era5_year.index, np.where(df_era5_year["labels"] == 1.0, df_era5_year["values"], None), color="red", label="ERA5")
    plt.plot(df_simu_year.index, np.where(df_simu_year["labels"] == 1.0, df_simu_year["values"], None), color="blue", label=simu)
    plt.plot(df_simu_bc_year.index, np.where(df_simu_bc_year["labels"] == 1.0, df_simu_bc_year["values"], None), color="green", label=simu)
    plt.plot(df_era5_year.index, np.where(df_era5_year["labels"] == 2.0, df_era5_year["values"], None), color="red")
    plt.plot(df_simu_year.index, np.where(df_simu_year["labels"] == 2.0, df_simu_year["values"], None), color="blue")
    plt.plot(df_simu_bc_year.index, np.where(df_simu_bc_year["labels"] == 2.0, df_simu_bc_year["values"], None), color="green")
    plt.plot(df_simu_year.index, np.where(df_simu_year["labels"] == 3.0, df_simu_year["values"], None), color="blue")
    plt.plot(df_simu_bc_year.index, np.where(df_simu_bc_year["labels"] == 3.0, df_simu_bc_year["values"], None), color="green")
    ...
    ds_train_hist_bc = xr.Dataset(
        data_vars=dict(tas=(["time", "lat", "lon"], train_hist_bc)), coords=dict(lat=("lat", lat), lon=("lon", lon), time=("time", train_hist["dates"]))
    )
    ds_test_hist_bc = xr.Dataset(
        data_vars=dict(tas=(["time", "lat", "lon"], test_hist_bc)), coords=dict(lat=("lat", lat), lon=("lon", lon), time=("time", test_hist["dates"]))
    )

    ds_test_future_bc = xr.Dataset(
        data_vars=dict(tas=(["time", "lat", "lon"], test_future_bc)), coords=dict(lat=("lat", lat), lon=("lon", lon), time=("time", test_future["dates"]))
    )
    if simu == "gcm":
        os.makedirs(GCM_RAW_DIR / "CNRM-CM6-1-BC", exist_ok=True)
        ...
        os.makedirs(RCM_RAW_DIR / "ALADIN-BC", exist_ok=True)
    ...
    for date in DATES_BC_TRAIN_HIST:
        ...
        ds_train_hist_bc_i = reformat_as_target(ds_train_hist_bc_i, target_file=target_file, domain=domain, method="conservative_normed", mask=True)
    ...
    for date in DATES_BC_TEST_HIST:
        ...
        ds_test_hist_bc_i = reformat_as_target(ds_test_hist_bc_i, target_file=target_file, domain=domain, method="conservative_normed", mask=True)
    ...
    for date in DATES_BC_TEST_FUTURE:
        ...
        ds_test_future_bc_i = reformat_as_target(ds_test_future_bc_i, target_file=target_file, domain=domain, method="conservative_normed", mask=True)
