...
from iriscc.settings import COLORS, CONFIG, GCM_RAW_DIR, GRAPHS_DIR, PREDICTION_DIR, RCM_RAW_DIR
...
if __name__ == "__main__":
    ...
    inputs_projections = [ccrs.PlateCarree(), CONFIG[exp]["data_projection"], CONFIG[exp]["data_projection"]]
    if CONFIG[exp]["target"] == "safran":
        domain = [CONFIG[exp]["domain"], CONFIG[exp]["domain_xy"], CONFIG[exp]["domain_xy"]]
    else:
        domain = [CONFIG[exp]["domain"], CONFIG[exp]["domain"], CONFIG[exp]["domain"]]
    if simu == "rcm":
        simu_name = "RCM 12km"
    ...
    fig1, axes1 = plt.subplots(3, 3, figsize=(15, 10), subplot_kw={"projection": output_projection})

    fig2, axes2 = plt.subplots(1, 3, figsize=(15, 5))
    fig3, axes3 = plt.subplots(1, 3, figsize=(15, 5))

    ## Reference 1980-2010
    data_ref = xr.open_mfdataset(glob.glob(str(dir / "tas*historical_r1i1p1f2*.nc"))).sel(time=slice("1980", "2010"))
    ...
    for i in range(len(periods) - 1):
        ...
        for col in range(3):
            ...
            ax1.set_extent([-5.0, 11.0, 41.0, 51.0], crs=ccrs.PlateCarree())
