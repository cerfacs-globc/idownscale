...
def filter_aberrant_values(param: np.ndarray, threshold: float) -> np.ndarray:
    ...
    for t, i, j in zip(*indices):
        param_cell = np.mean(param[t, max(0, i - 1) : min(param.shape[1], i + 2), max(0, j - 1) : min(param.shape[2], j + 2)])
...
if __name__ == "__main__":
    ...
    beta = np.array(beta)
    alpha = filter_aberrant_values(alpha, 8 * np.nanstd(alpha, where=alpha > 0))
    beta = filter_aberrant_values(beta, 8 * np.nanstd(beta, where=beta > 0))
