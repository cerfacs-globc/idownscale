...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(self.logit_scale, max=torch.log(torch.tensor(1.0 / 0.01)).to(self.logit_scale.device)).exp()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim, norm_layer=norm_layer if self.patch_norm else None
        )
        ...
        if self.upsampler == "pixelshuffle":
            ...
            self.conv_before_upsample_hf = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1), nn.LeakyReLU(inplace=True))
