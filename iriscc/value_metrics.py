...
def get_temporal_metrics(obs, pred):
    ...
    return {"autocorr_obs": lag1_autocorr(obs), "autocorr_pred": lag1_autocorr(pred), "autocorr_error": lag1_autocorr(pred) - lag1_autocorr(obs)}
...
def get_spatial_metrics(obs_mean_map, pred_mean_map):
    ...
    return {"spatial_corr": pearsonr(obs_flat[mask], pred_flat[mask])[0], "spatial_rmse": np.sqrt(np.nanmean((pred_mean_map - obs_mean_map) ** 2))}
