...
def standardize_dims_and_coords(ds):
    ...
    dim_mapping = {"x": ["i", "ni", "xh", "lon", "nlon"], "y": ["j", "nj", "yh", "lat", "nlat"], "lev": ["olevel"]}
    coord_mapping = {"lon": ["longitude", "nav_lon"], "lat": ["latitude", "nav_lat"]}
...
def add_lon_lat_bounds(ds: xr.Dataset, projection=None, bounds_method="1") -> xr.Dataset:
    ...
    if bounds_method == "1":
        ...
        ds = ds.assign_coords(x_b=("x_b", x_b), y_b=("y_b", y_b), lon_b=(["y_b", "x_b"], lon_b), lat_b=(["y_b", "x_b"], lat_b))
    ...
    if bounds_method == "2":
      ...
      """
        ...
        ds = ds.assign_coords(lon_b=(["y_b", "x_b"], lon_b_full), lat_b=(["y_b", "x_b"], lat_b_full))
...
def interpolation_target_grid(ds: xr.Dataset, ds_target: xr.Dataset, method: str, input_projection=None, target_projection=None, bounds_method="1") -> xr.Dataset:
    """
...
def reformat_as_target(
    ...
    ds = interpolation_target_grid(ds, ds_target, method, input_projection, target_projection)
    return ds
...
def apply_landseamask(ds: xr.Dataset, mask_type: str, variables, domain=None) -> xr.Dataset:
    ...
    if mask_type == "gcm":
        ...
        mask = standardize_dims_and_coords(mask)
        condition = mask["landseamask"].values == 1.0  # Land-sea mask value
    else:
        raise ValueError("Invalid mask_type. Choose from 'gcm', 'era5', or 'eobs'.")

    # Align mask with the dataset's spatial domain
    mask = mask.sel(lon=slice(ds["lon"].values.min(), ds["lon"].values.max()), lat=slice(ds["lat"].values.min(), ds["lat"].values.max()))
