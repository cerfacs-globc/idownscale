...
def standardize_cerra_geometry(ds):
    if "latitude" in ds.coords:
        ds = ds.rename({"latitude": "lat"})
    if "longitude" in ds.data_vars:
        ds = ds.set_coords("longitude").rename({"longitude": "lon"})
    if "lon" in ds.coords:
...
def reformat_as_target(ds:xr.Dataset,
    ...
    # Standardize the target according to its actual grid structure.
    target_hint = str(target_file).lower() if isinstance(target_file, (str, Path)) else ""
    if "safran" in target_hint:
       ds_target = ARCHIVAL_standardize_dims_and_coords(ds_target)
    elif ("lon" in ds_target.coords and "lat" in ds_target.coords
          and ds_target.lon.ndim == 2 and ds_target.lat.ndim == 2):
       ds_target = standardize_cerra_geometry(ds_target)
    else:
...
def crop_domain_from_ds(ds:xr.Dataset, domain:tuple) -> xr.Dataset:
   ...
   if domain:
      if "lon" in ds.coords:
         ...
         domain_mask = mask_lon & mask_lat
         # Map coordinate masks to correct dimension handles (x/y)
         dim_x = "x" if "x" in ds.dims else "lon"
         dim_y = "y" if "y" in ds.dims else "lat"
         if getattr(mask_lon, "ndim", 1) == 2:
            mask_lon = np.any(mask_lon, axis=0)
         if getattr(mask_lat, "ndim", 1) == 2:
            mask_lat = np.any(mask_lat, axis=1)
         ds = ds.isel({dim_x: mask_lon, dim_y: mask_lat})
         if getattr(domain_mask, "ndim", 1) == 2:
            domain_mask = domain_mask.isel({dim_x: mask_lon, dim_y: mask_lat})
            for var in ds.data_vars:
               if ds[var].dims[-2:] == (dim_y, dim_x):
                  ds[var] = ds[var].where(domain_mask)
      else:
...
class Data:
   ...
   def _resolve_source_files(self, source_name: str, var: str, date=None, ssp: str | None = None) -> list[str]:
      ...
      elif date is None or date < pd.Timestamp("2015-01-01"):
      ...
      return list(np.sort(np.array(candidates)))

   def _resolve_source_file(self, source_name: str, var: str, date=None, ssp: str | None = None) -> str:
      spec = self.get_source_spec(source_name)
      candidates = self._resolve_source_files(source_name, var, date=date, ssp=ssp)
   ...
   def _open_source_dataset(self, source_name: str, var: str, date=None, ssp: str | None = None) -> xr.Dataset:
      ...
      if spec.get("combine_files_by") == "time":
         files = self._resolve_source_files(source_name, var, date=date, ssp=ssp)
         datasets = []
         for file in files:
            ds_i = xr.open_dataset(file, engine="netcdf4")
            ds_i = self._standardize_source_geometry(ds_i, spec.get("geometry", "none"))
            if date is not None and "time" in ds_i.dims:
               mask = (
                  (ds_i.time.dt.year == date.year)
                  & (ds_i.time.dt.month == date.month)
                  & (ds_i.time.dt.day == date.day)
               )
               if int(mask.sum()) == 0:
                  ds_i.close()
                  continue
               ds_i = ds_i.isel(time=mask)
            datasets.append(ds_i)
         if not datasets:
            raise FileNotFoundError(
               f"Missing matching time slices for source='{source_name}', var='{var}', date='{date}', ssp='{ssp}'"
            )
   ...
   def get_observation_dataset(self, source_name: str, var: str, date, skip_domain_crop: bool = False):
      spec = self.get_source_spec(source_name)
      ds = self._open_source_dataset(source_name, var, date=date)
      ds = self.crop_time_dim(ds, date)
   ...
   def get_target_dataset(
      ...
      skip_domain_crop: bool = False,
   ) -> xr.Dataset:
      if source_name is not None:
         ds = self.get_observation_dataset(source_name, var, date, skip_domain_crop=skip_domain_crop)
      elif target == "safran":
         ds = self.get_safran_dataset(var, date)
      elif target == "eobs":
         ds = self.get_eobs_dataset(var, date)
      elif target in SOURCE_CATALOG and self.get_source_spec(target).get("kind") == "observation":
         ds = self.get_observation_dataset(target, var, date, skip_domain_crop=skip_domain_crop)
      return ds
